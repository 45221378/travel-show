<template>
  <div class="">
    <div>
      <img crossOrigin="anonymous" :src="imgSrc" @click="showImgTrue">
    </div>
    <div class="hint-img" v-if="showImg">
      <div class="viewerWrap">
        ...
        <div class="imgAndAnnotateWrap" :style="'left:'+ WrapStyle.imgLeft+'px; top:'+ WrapStyle.imgTop+'px; width:' +WrapStyle.imgWidth +'px; height:'+WrapStyle.imgHeight+'px'">
          <img crossOrigin="anonymous" :title="showPercent" width="100%" height="100%" ref="imgHolder" :style="imgStyle" @mousedown="dragImgMove" @mouseup="removeDrag" ondragstart="return false;" class="showIMG" :src="imgSrc" @mousewheel="rollImg">
          <div v-show="editFlag" class="annotateWrap" ref="canvasWrap">
      ...
      <div class="viewer-footer">
        <div class="viewer-toolbar" v-if="!editFlag">
          <ul>
            <li class="viewer-zoom-in" data-viewer-action="zoom-in" @click="bigger"></li>
            <li class="viewer-zoom-out" data-viewer-action="zoom-out" @click="smaller"></li>
            <li class="viewer-reset" data-viewer-action="reset" @click="reset"></li>
            <li class="viewer-rotate-left" data-viewer-action="rotate-left" @click="rotateleft"></li>
            <li class="viewer-rotate-right" data-viewer-action="rotate-right" @click="rotateright"></li>
            <p class="img-btn" @click="edit">
              批注
            </p>
          </ul>
...
import { dragImgMove, removeDrag } from "@/lib/commonMethods.js";
import { EraserBrush } from "./brush";
export default {
  name: "app-handleImg",
  props: ["imgSrc"],
  data() {
  ...
  methods: {
    ...
    reset() {
      this.scaleNum = 1;
      this.rotateNum = 0;
      this.$refs.imgHolder.style = "position:static";
    },
    rotateleft() {
      event.preventDefault();
      this.rotateNum += 90;
      ...
      if (this.editFlag) {
        ...
        this.$nextTick(() => {
          ...
          if (this.$refs.canvasWrap) {
            let objectCanvas = this.$refs.canvasWrap.getElementsByTagName(
              "canvas"
            );
