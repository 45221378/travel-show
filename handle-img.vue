<template>
  <div class="">
    <div>
      ...
      <img ref="imgHoldeCopy" title="查看批注图片" crossOrigin="anonymous" :src="simageURL+'?time=' + new Date().valueOf()" style="display:none">
    </div>
    <div class="hint-img" v-if="showImg">
      ...
      <div class="handle-footer">
        <!-- 操作图片的工具栏 --> 
        <div class="handle-toolbar" v-if="editFlag">
          <ul class="clearfix">
            <!-- 挂载的调节大小和颜色的区域模块 -->
            <div class="mount">
  ...
  methods: {
    ...
    showImgTrue() {
      ...
      this.$nextTick(() => {
        ...
        this.canvas && this.editImg();
        this.canvas && this.pressImg();
      });
    },
    editImg() {
      this.editFlag = true;
    ...
    imgToCanvas() {
      if (this.canvas && this.imgElement) {
        const img = new fabric.Image(this.$refs.imgHoldeCopy);
        this.canvas.clear();
        this.canvas.add(img);
    ...
    setDragEvent(event) {
      ...
      if (
        ...
        this.$refs.canvasWrap.style.zIndex = -1;
        //空格会与写字的空格冲突, 强制性失焦把文字写上去
        this.showTextArea && this.whriteTextToCanvas();
      }
    },
    ...
    // 压缩图片
    pressImg() {
      const { naturalHeight, naturalWidth, src } = this.$refs.imgHoldeCopy;
    ...
    initCanvas() {
      //获取图片的宽高，并且赋值给canvas
      this.imgElement = this.$refs.imgHoldeCopy;
      const { naturalHeight, naturalWidth } = this.$refs.imgHoldeCopy;
